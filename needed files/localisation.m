...
while(converged == 0 && n < maxNumOfIterations) %%particle filter loop
    ...
    if(path_plan == 0)
        ...
        %else check if close to a wall and move away from it
       if(min(botScan) < 3) && (botScan(1) < 10)
           
%           if(botScan(3*readings/4+1) < botScan(readings/4+1))
%              angle = tan(-1) * (botScan(1)/botScan(3*readings/4+1)) + (90*pi/180); 
%           elseif (botScan(readings/4+1) < botScan(3*readings/4+1))
%              angle = tan(-1) * (botScan(readings/4+1) / botScan(1));
%           end 

            [min_val min_val_id] = min(botScan);
            if(min_val_id < readings/2)
                turn = -(pi/16);
            else
                turn = pi/16;
            end
            ...
            botSim.turn(turn);
            flag_turn = 1;
       elseif (min(botScan) < 3) && (botScan(1) > 15)
            move = 5;
            botSim.move(move);
            flag_move = 1;
       elseif (localise == 0)
    ...
    for i=1:num  
        ...
        if ((sigma > (noiselevel*0.8+ 5)*readings)&&(n>3)&&(flag_random == 0)&&(path_plan == 0))||(reinit_count > 4)
            particles(i).randomPose(1);
...
%         end
    ...
    disp(['X Y theta: ' num2str(particles(min_index).getBotPos(1), 3) ' ' num2str(particles(min_index).getBotPos(2), 3) ' ' num2str(particles(min_index).getBotAng(), 5)]);
end
end
