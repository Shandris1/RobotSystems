function [reduced, flag_cw]= reduce_graph(map,dist)
...
counter = 1;
    ...
    for i = 1: length(vertices)
        ...
        elseif (flag_cw)
          ...
          if(i ==1)
          reduced(counter,:) = vertices(i,1:2) - abs(dist/sin(check_ang(1,1)))*vector - abs(dist*(1/sin(check_ang(1,1))-1)/cos(check_ang(1,1)))*(vector_edges(end,:));
           counter = counter +1;
          else
           reduced(counter,:) = vertices(i,1:2) - (dist/abs(sin(check_ang(1,1))))*vector - abs(dist*(1/abs(sin(check_ang(1,1)))-1)/abs(cos(check_ang(1,1))))*(vector_edges(i-1,:));
           counter = counter +1;
          end
          reduced(counter,:) = vertices(i,1:2) - (dist/abs(sin(check_ang(1,1))))*vector + abs(dist*(1/abs(sin(check_ang(1,1)))-1)/abs(cos(check_ang(1,1))))*(vector_edges(i,:));
         %reduced(counter,:) = vertices(i,1:2) - abs(dist*(1/sin(check_ang(1,1))-1)/cos(check_ang(1,1)))*vector;
          counter = counter +1;
        ...
        elseif(flag_cw==0)&&(check_ang(1,1) > pi) 
         reduced(counter,:) = vertices(i,1:2) + abs(dist/sin(check_ang(1,1)))*vector;
          ...
          reduced(counter,:) = vertices(i,1:2) - abs(dist/sin(check_ang(1,1)))*vector - abs(dist*(1/sin(check_ang(1,1))-1)/cos(check_ang(1,1)))*(vector_edges(end,:));
           ...
           reduced(counter,:) = vertices(i,1:2) - abs(dist/sin(check_ang(1,1)))*vector - abs(dist*(1/sin(check_ang(1,1))-1)/cos(check_ang(1,1)))*(vector_edges(i-1,:));
           counter = counter +1;
          end    
          reduced(counter,:) = vertices(i,1:2) - abs(dist/sin(check_ang(1,1)))*vector + abs(dist*(1/sin(check_ang(1,1))-1)/cos(check_ang(1,1)))*(vector_edges(i,:));
         %reduced(counter,:) = vertices(i,1:2) - abs(dist*(1/sin(check_ang(1,1))-1)/cos(check_ang(1,1)))*vector;
          counter = counter +1;
