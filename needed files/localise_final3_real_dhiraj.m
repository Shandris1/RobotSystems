...
%             %turn_long = 2*pi/readings*(max_ang_ind-1)+rand(1)*pi/18;
            ...
            sort_botscan = sort(botScan_move);

            found = 0;
            count = 1;
            while (found == 0) 
                [top_value top_ind] = max(sort_botscan);
                
                if (top_ind-1 == 0)
                    top_ind = readings;
                elseif (top_ind+1 == readings+1)
                    top_ind = 1;
                end
                
                if(botScan_move(top_ind)*0.5 > botScan_move(top_ind-1) && botScan_move(top_ind)*0.5 > botScan_move(top_ind+1))
                    found = 1;
                    move = 5;
