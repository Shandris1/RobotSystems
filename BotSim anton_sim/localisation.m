...
search_long = 1;
flag_random = 0;
random_move = round(7/330*max_dim);
noise = noiselevel + 7;
...
while(converged == 0 && n < maxNumOfIterations) %%particle filter loop
    ...
    % get the standard deviation for X Y and theta
    ...
    for i=1:num  
        ...
        %readings in order to assign weights to particles
...
%             end
    ...
    if(6*sigmax < 0.3*max_dim)&&(6*sigmay < 0.3*max_dim)||(path_plan)
        blobcheck = 1;
        sigma_sense = 2 + noiselevel/4;
    else
        blobcheck = 0;
