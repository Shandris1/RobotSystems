function [mean_particle, particles, mean_pos mean_ang] = localise_final3_real(botSim,mean_particle,chosen_particles,particles,map,target,readings, noiselevel,num,path_plan,max_dim_vec,moveCmd,turnCmd,optimal,localise,stepsize,adjacency,reduced,path_dist)
...
%         
%             if(path_plan == 0)
%            for j =0 :readings_num-1
%            check_orient(j+1) = sum((abs(circshift(par_sense,j) - bot_sense)));
%             end
%          [lowest, best] = min((check_orient));
% %        %dist(i) = sqrt(sum((botScan - circshift(parScan,best - 1)) .^ 2)); 
%          if(lowest < closest*readings_num)
%             best_real = vector_indices(best);
%              particles(i).turn(-2*pi/readings * (best_real-1));      
%              par_sense = circshift(par_sense,best-1);
%             counting = counting +1;
%             circ_ind(counting) = i;
%          end
%          vector_indices  = [];
...
%         distScan = botScan;
%         distScan(badReadings) = []
%         parScan(badReadings) = []
        %dist(i)= sum((abs(parScan - distScan)));
...
%         botScan(badReadings) = [];
%         sigma = sum((abs(measurement - botScan)))
        best_flag = 3;
        damping = 0;
    else
        blobcheck = 0;
    ...
    bot_sense(bad_meas) = [];
        ...
        if(sum((abs(par_sense - bot_sense)))< (0.1+noiselevel*0.6)*readings_num)
               if(path_plan)
                   if(sum((abs(par_sense - bot_sense)))< (0.1+noiselevel*0.2)*readings_num)
                    localise = 1;
