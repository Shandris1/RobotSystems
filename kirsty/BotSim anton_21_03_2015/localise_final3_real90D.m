function [mean_particle, particles, mean_pos mean_ang] = localise_final3_real(botSim,mean_particle,chosen_particles,particles,map,target,readings, noiselevel,num,path_plan,max_dim_vec,moveCmd,turnCmd,optimal,localise,stepsize,adjacency,reduced,dist)
...
%     end
        ...
        if(path_plan == 0)
            ...
            left = (botScan_move(goodLeftReadings) < 14);
            right = (botScan_move(goodRightReadings) < 14);
        end
...
%     botScan_move(satur_ind) = saturation_value;
    ...
    for i=1:num  
        ...
        if ((sigma > (noiselevel*best_flag+ 5)*readings_update)&&(n>3)&&(flag_random == 0))
            particles(i).randomPose(1);
...
%         
       ...
       weight(i) = exp((-(error)^2)/(2*(sigma_sense*readings_update*0.5)^2)) + damping;
...
%     end
    ...
    if((5*sigmax < 0.3*max_dim)&&(5*sigmay < 0.3*max_dim))||(path_plan)
        blobcheck = 1
        sigma_sense = 2 + noiselevel/4;
