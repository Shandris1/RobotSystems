...
%     end
        ...
        if(search_long)
           ...
           if( max_ang_ind == (readings_update/2 +1))
            ...
            turn = 2*pi/readings_update*(max_ang_ind-1)+rand(1)*pi/18;
            turning(speed,turn);
            search_long = 0;
            move = 5;
            max_found = 1;
...
%         end
    ...
    readings_update = readings_update - length(badReadings);
    botScan(badReadings) = [];
    ...
    for i=1:num  
        ...
        if (sigma > (noiselevel*0.8+ 5)*readings_update)&&(n>3)&&(flag_random == 0)
            particles(i).randomPose(1);
