function [botSim, mean_pos mean_ang] = localise_final(botSim,map,target)
...
while(converged == 0 && n < maxNumOfIterations) %%particle filter loop
    ...
    if(sigma > 60) && (n > 1)
        botSim.turn(mean(par_ang));
    end
    
    botSim.move(move); %move the real robot. These movements are recorded for marking 
    
    if (converge_flag == 1)
        botSim.turn(mean_ang);
    end
    ...
    for i =1:num %for all the particles. 
        ...
        if(sigma > 60) && (n > 1)
            ...
            x = particles(i).getBotAng - mean(par_ang);
            x = x + particles(i).getBotAng;
            particles(i).turn(x);
        else 
            if(flag == 1)
        ...
        if (converge_flag == 1)
            particles(i).turn(mean_ang);  
        end
        
        %save all pos and ang 
    ...
    %%
    if(6*sigmax < 15)&&(6*sigmay < 15)&&(6*sigmaang < 50/180*pi)
