function [mean_particle, particles, mean_pos mean_ang] = localise_final3_real90D(botSim,mean_particle,chosen_particles,particles,map,target,readings, noiselevel,num,path_plan,max_dim_vec,moveCmd,turnCmd,optimal,localise,stepsize,adjacency,reduced,dist)
...
%     end
        ...
        elseif (path_plan == 0) && (sum(left) < 1) && (sum(right) < 1)
            if(search_long == 0 && min([botScan_move(1) botScan_move(2) botScan_move(readings)]) > wall_follow_dist) 
                move = 7;
                driving(speed,move);
                search_long = 0;
                max_found = 0;
